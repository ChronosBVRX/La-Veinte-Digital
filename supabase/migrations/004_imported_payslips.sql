-- Imported Payslips Schema
-- Requires: auth.users, public.profiles, public.payroll_contexts
--
-- Tarjetones IMSS confirmados por el trabajador. El PDF NUNCA se sube:
-- solo llega el resultado estructurado y la huella SHA-256 del archivo
-- (source_hash) para detectar duplicados. No se guardan RFC, CURP, NSS,
-- cuenta bancaria, folio fiscal, códigos QR ni sellos: solo folio de
-- control, huella del folio fiscal y datos laborales del recibo.

-- 1. Imported payslips (one row per confirmed receipt)
CREATE TABLE IF NOT EXISTS public.imported_payslips (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
  source_hash TEXT NOT NULL,
  extraction_method TEXT NOT NULL CHECK (extraction_method IN ('native_text', 'ocr', 'hybrid')),
  period_raw TEXT NOT NULL DEFAULT '',
  period_year SMALLINT,
  period_month SMALLINT,
  period_half SMALLINT,
  folio TEXT,
  fiscal_folio_hash TEXT,
  certification_date DATE,
  global_confidence NUMERIC(4, 3) NOT NULL DEFAULT 0 CHECK (global_confidence >= 0 AND global_confidence <= 1),
...
-- 2. Payslip concept lines (percepciones y deducciones)
CREATE TABLE IF NOT EXISTS public.imported_payslip_lines (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  payslip_id UUID NOT NULL REFERENCES public.imported_payslips(id) ON DELETE CASCADE,
  ...
  UNIQUE (payslip_id, line_index)
);

-- 3. Payslip observations
CREATE TABLE IF NOT EXISTS public.imported_payslip_observations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  payslip_id UUID NOT NULL REFERENCES public.imported_payslips(id) ON DELETE CASCADE,
  line_index SMALLINT NOT NULL,
  concept_code TEXT NOT NULL DEFAULT '',
  amount NUMERIC(14, 2),
  due_period TEXT,
  units SMALLINT,
  control_number TEXT,
  initial_charge NUMERIC(14, 2),
  notes TEXT,
  UNIQUE (payslip_id, line_index)
);

-- Enable RLS
ALTER TABLE public.imported_payslips ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.imported_payslip_lines ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.imported_payslip_observations ENABLE ROW LEVEL SECURITY;

-- RLS policies: users can only read/insert their own payslips
CREATE POLICY "Users can read own imported payslips"
  ON public.imported_payslips FOR SELECT
  TO authenticated
  USING (user_id = auth.uid());

CREATE POLICY "Users can insert own imported payslips"
  ON public.imported_payslips FOR INSERT
  TO authenticated
  WITH CHECK (user_id = auth.uid());

CREATE POLICY "Users can read own imported payslip lines"
  ON public.imported_payslip_lines FOR SELECT
  TO authenticated
  USING (payslip_id IN (SELECT id FROM public.imported_payslips WHERE user_id = auth.uid()));

CREATE POLICY "Users can insert own imported payslip lines"
  ON public.imported_payslip_lines FOR INSERT
  TO authenticated
  WITH CHECK (payslip_id IN (SELECT id FROM public.imported_payslips WHERE user_id = auth.uid()));

CREATE POLICY "Users can read own imported payslip observations"
  ON public.imported_payslip_observations FOR SELECT
  TO authenticated
  USING (payslip_id IN (SELECT id FROM public.imported_payslips WHERE user_id = auth.uid()));

CREATE POLICY "Users can insert own imported payslip observations"
  ON public.imported_payslip_observations FOR INSERT
  TO authenticated
  WITH CHECK (payslip_id IN (SELECT id FROM public.imported_payslips WHERE user_id = auth.uid()));

-- 4. Confirmación transaccional de un tarjetón
--
-- Inserta encabezado + líneas + observaciones y, en la MISMA transacción,
-- aplica al perfil (solo los campos autorizados por el trabajador) y
-- actualiza el contexto de nómina (categoría, jornada, antigüedad,
-- conceptos recurrentes y hechos de nómina).
--
-- No recibe el PDF: solo el resultado estructurado y la huella.
...
BEGIN
  ...
  IF (v_emp->>'employeeNumber') IS NOT NULL AND (v_emp->>'employeeNumber') <> '' THEN
    SELECT matricula INTO v_profile.matricula FROM public.profiles WHERE id = v_user_id;
    IF v_profile.matricula IS NOT NULL AND v_profile.matricula <> '' AND v_profile.matricula <> (v_emp->>'employeeNumber') THEN
      IF (p_profile_updates->>'matricula') IS DISTINCT FROM 'true' THEN
        RAISE EXCEPTION 'matricula_mismatch';
      END IF;
    END IF;
  END IF;

  -- Duplicados (misma huella del archivo para el mismo usuario).
  ...
  -- Contexto de nómina: categoría, jornada, antigüedad efectiva, tipo de
  -- contratación, conceptos recurrentes del tarjetón y hechos de nómina.
  ...
  -- Códigos recurrentes confirmados: 050 (mayor importe), 023 y 063.
  FOR v_line IN SELECT * FROM jsonb_array_elements(v_lines) LOOP
    v_code := v_line->>'code';
    ...
    END LOOP;
    IF v_found THEN
      -- Actualizar importe y última aparición del código existente.
      v_new_rc := (
        SELECT COALESCE(jsonb_agg(
          CASE WHEN entry->>'conceptCode' = v_code THEN
            jsonb_set(jsonb_set(
              entry,
              ARRAY['lastAmount'], to_jsonb((v_line->>'amount')::NUMERIC)
            ), ARRAY['lastSeenAt'], to_jsonb(NULLIF(v_doc->>'periodRaw', '')))
          ELSE entry END
        ), '[]'::jsonb) FROM jsonb_array_elements(v_new_rc) AS entry
      );
    ELSIF (v_line->>'confirmedByUser')::BOOLEAN THEN
      v_new_rc := v_new_rc || jsonb_build_object(
        'conceptCode', v_code,
        'appearsNormally', true,
        'lastAmount', (v_line->>'amount')::NUMERIC,
        'source', 'last_payslip',
        'firstSeenAt', NULLIF(v_doc->>'periodRaw', ''),
        'lastSeenAt', NULLIF(v_doc->>'periodRaw', ''),
        'confirmed', true
      );
    END IF;
  END LOOP;
  ...
  IF v_has_054 THEN
    ...
    IF NOT v_found THEN
      v_facts := v_facts || jsonb_build_object(
        ...
        'value', true,
        'source', 'imported_payslip',
        'period', NULLIF(v_doc->>'periodRaw', '')
      );
    END IF;
  END IF;

  INSERT INTO public.payroll_contexts (
    user_id, category_name, workday_hours, employment_type,
    effective_seniority_date, recurring_concepts, payroll_facts, updated_at
  ) VALUES (
    v_user_id, v_cat_name,
    CASE WHEN v_hours IN (6, 6.5, 8, 12) THEN v_hours ELSE NULL END,
    v_employment, v_sen_date, v_new_rc, v_facts, now()
  )
...
REVOKE ALL ON FUNCTION public.confirm_imported_payslip(TEXT, JSONB, JSONB, BOOLEAN) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.confirm_imported_payslip(TEXT, JSONB, JSONB, BOOLEAN) TO authenticated;
