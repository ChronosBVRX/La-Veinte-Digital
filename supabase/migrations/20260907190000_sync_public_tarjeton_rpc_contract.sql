-- =============================================================================
-- Migración: 20260907190000_sync_public_tarjeton_rpc_contract.sql
-- Propósito:
--   1. Sincronizar el wrapper público confirm_imported_payslip con el contrato
--      TypeScript completo de ParsedImssTarjeton / ConfirmTarjetonRequest.
--   2. Extender la whitelist de vacations para incluir porVencer, porVencerRaw y dueDate.
--   3. Hacer tolerante la validación de líneas tanto para confirmedByUser (camelCase)
--      como confirmed_by_user (snake_case).
--   4. Mantener la delegación canónica y atómica a confirm_imported_payslip_v1.
-- =============================================================================
...
BEGIN
  ...
  -- Defensa en profundidad para invocaciones directas al RPC
  ...
  LOOP
    IF COALESCE((v_line->>'confirmedByUser')::BOOLEAN, (v_line->>'confirmed_by_user')::BOOLEAN, false) <> true THEN
