-- ============================================================================
-- Migración: 20260907150000_radiological_exposure_provenance_and_duplicate_reconcile.sql
--
-- 1. Añade columnas de procedencia y auditoría de exposición radiológica en vacation_profile_data
-- 2. Limpieza reparadora de datos legacy sin procedencia (evita heredar YES a ciegas)
-- 3. Actualiza confirm_imported_payslip_v1:
--    - Separa DOCUMENT INSERT de ACTIVE WORKER RECONCILIATION.
--    - En duplicados, no inserta payslip/líneas duplicadas pero SÍ reconcilia perfil,
--      payroll_contexts y vacation_profile_data (reseteando en workerReplacement).
--    - Registra radiological_exposure_source ('USER_CONFIRMED', 'ACTIVE_PAYSLIP', 'LEGACY').
-- ============================================================================

-- 1. Columnas de procedencia en vacation_profile_data
ALTER TABLE public.vacation_profile_data
  ADD COLUMN IF NOT EXISTS radiological_exposure_source TEXT DEFAULT 'LEGACY',
  ADD COLUMN IF NOT EXISTS radiological_exposure_updated_at TIMESTAMPTZ DEFAULT now();

-- Constraint condicional para la fuente
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'vacation_profile_data_radiological_exposure_source_check'
  ) THEN
    ALTER TABLE public.vacation_profile_data
      ADD CONSTRAINT vacation_profile_data_radiological_exposure_source_check
      CHECK (radiological_exposure_source IN ('USER_CONFIRMED', 'ACTIVE_PAYSLIP', 'LEGACY'));
  END IF;
END $$;

-- 2. Limpieza reparadora de datos legacy
-- Marcar registros existentes sin fuente explícita como LEGACY
UPDATE public.vacation_profile_data
SET radiological_exposure_source = 'LEGACY'
WHERE radiological_exposure_source IS NULL;

-- Si un registro tiene radiological_exposure = 'YES' pero proviene de un backfill o estado legacy sin USER_CONFIRMED,
-- convertirlo a NULL para que el tarjetón activo de la matrícula determine su estado de forma limpia.
UPDATE public.vacation_profile_data
SET radiological_exposure = NULL,
    radiological_exposure_source = 'LEGACY',
    radiological_exposure_updated_at = now()
WHERE radiological_exposure = 'YES'
  AND (radiological_exposure_source IS NULL OR radiological_exposure_source = 'LEGACY');

-- 3. Reemplazo de RPC confirm_imported_payslip_v1
...
BEGIN
  ...
  v_method := v_doc->>'extractionMethod';
  v_confidence := COALESCE((v_doc->>'globalConfidence')::NUMERIC, 0);

  IF v_method NOT IN ('native_pdf', 'ocr_client', 'ocr_external', 'manual_assisted') THEN
  ...
  FOR v_line IN SELECT * FROM jsonb_array_elements(v_lines) LOOP
    IF (v_line->>'kind') = 'earning' THEN
      ...
      IF (v_line->>'code') = '054' AND (v_line->>'amount')::NUMERIC > 0 THEN
        v_has_054 := true;
      END IF;
  ...
  IF v_existing IS NOT NULL THEN
    -- DOCUMENTO DUPLICADO: omitir inserción física de payslip y líneas, pero continuar con la reconciliación activa
    v_id := v_existing;
    v_is_duplicate := true;
  ...
  -- ==========================================================================
  -- RECONCILIACIÓN DEL TRABAJADOR ACTIVO (se ejecuta SIEMPRE, incluso en duplicados)
  -- ==========================================================================

  -- 1. Perfil: actualizar únicamente campos autorizados
  ...
  IF v_worker_replacement THEN
    ...
    INSERT INTO public.vacation_profile_data (
      ...
      v_user_id, v_new_matricula, v_cat_name, v_emp->>'categoryCode', v_emp->>'location',
  ...
  ELSE
    ...
    INSERT INTO public.vacation_profile_data (
      ...
      v_user_id, COALESCE(v_new_matricula, v_old_matricula), v_cat_name, v_emp->>'categoryCode', v_emp->>'location',
      NULLIF(v_emp->>'entryDate', '')::DATE, (v_seniority->>'years')::INTEGER, (v_seniority->>'fortnights')::INTEGER,
      (v_seniority->>'days')::INTEGER, CASE WHEN v_has_054 THEN 'YES' ELSE 'NO' END, 'ACTIVE_PAYSLIP',
      now(), '{5,6}', now(), now()
    )
