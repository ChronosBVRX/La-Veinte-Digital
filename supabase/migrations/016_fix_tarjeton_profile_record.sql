...
-- Corrige acceso prematuro a v_profile RECORD que no ha sido inicializado.
-- PostgreSQL requiere que un RECORD reciba una fila completa antes de
-- acceder a cualquiera de sus campos.
--
-- La funcion v1 fue heredada de 004/012 y renombrada en 014. El wrapper
-- publico confirm_imported_payslip (de 014/015) delega en ella.
...
BEGIN
  ...
  -- Consentimiento explicito: sin el no hay persistencia, punto.
  IF p_authorize_server_storage <> true THEN
    RAISE EXCEPTION 'consent_required';
  END IF;

  -- Registra el consentimiento en la misma transaccion que el tarjeton.
  ...
  -- Recalcular totales en el servidor (nunca se confia en el cliente).
  v_sum_earn := 0;
  v_sum_ded := 0;
  ...
  -- Matricula: sin confirmacion explicita no se actualiza ni se acepta
  -- un cambio. Si el tarjeton trae matricula distinta a la del perfil y
  -- el trabajador no autorizo el cambio, se detiene la confirmacion.
