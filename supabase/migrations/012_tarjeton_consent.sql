...
-- El consentimiento de almacenamiento en servidor pasa a ser parte de la
-- MISMA transacción que guarda el tarjetón y actualiza el contexto de
-- nómina. Antes el cliente llamaba a grantPayrollConsent() por separado y,
-- si fallaba, la confirmación seguía adelante (fail-open).
--
-- Se elimina la firma anterior del RPC para que no quede ningún camino
-- que persista sin consentimiento.

drop function if exists public.confirm_imported_payslip(text, jsonb, jsonb, boolean);
...
BEGIN
  ...
  -- Consentimiento explícito: sin él no hay persistencia, punto.
  IF p_authorize_server_storage <> true THEN
    RAISE EXCEPTION 'consent_required';
  END IF;

  -- Registra el consentimiento en la misma transacción que el tarjetón.
