...
| 7 | `Lim Ocu` | Limite Ocupa, Fec Lim Ocu | `occupation_limit_date`, `occupation_limit_is_sentinel` | `date` / `bool` | **CONFIRMADO POR CATÁLOGO IMSS** | Fecha límite de ocupación de la plaza. La fecha `2050-01-01` constituye una fecha centinela del sistema SIAP y se marca con `occupation_limit_is_sentinel = true`. No debe interpretarse de forma aislada como certeza absoluta de plaza definitiva de base sin verificar `TC` y `Tipo de Plaza`. |
| 8 | `MO` | Mca Ocu, Marca Ocupacion | `occupation_mark_code` | `text` | **CÓDIGO INTERNO SIN CATÁLOGO PÚBLICO** | Marca administrativa interna del sistema SIAP (e.g. `0`). |
...
| 10 | `Turno` | Jor, Jornada, Tipo Jornada | `shift_code`, `turn` | `text` | **CONFIRMADO POR CATÁLOGO IMSS** | Código numérico de jornada/turno institucional. |
| 11 | `C A` | CA, Conceptos Asociados | `associated_concepts_mask`, `associated_concepts` | `text` / `jsonb` | **DEDUCIDO DEL ARCHIVO / PROCEDIMIENTO SIAP** | Bitmask de 5 posiciones derivada del procedimiento de actualización de plazas SIAP. Valores numéricos en Excel (ej. `1`, `10`, `100`) se rellenan con ceros a la izquierda a 5 caracteres (`00001`, `00010`, etc.). Decodifica a conceptos 012, 014, 023, 054, 063. |
...
| 21 | `Número de Seguridad Social` | N.S.S., NSS | `nss` | `text` | **CONFIRMADO POR NORMA** | Número de Seguridad Social. Se aceptan 10 u 11 dígitos (`^\d{10,11}$`). En el archivo canónico existen 322 registros con 10 dígitos (sin dígito verificador emitido por SIAP); se almacenan tal cual sin añadir ceros iniciales artificiales. |
...
| 27 | `Micro y grupo` | Micro Grupo, Microgrupo | `micro_group_code` | `text` | **CÓDIGO INTERNO SIN CATÁLOGO PÚBLICO** | Agrupación presupuestal y de planeación interna de personal del IMSS. |
...
2. **Nuevos Trabajadores Creados por el Lote:** Se localizan mediante `source_created_by_batch_id = p_batch_id` y se marcan con `source_rolled_back_at = now()`, `active = false` y nota explicativa en su expediente.
