...
| 8 | `MO` | Mca Ocu, Marca Ocupacion | `occupation_mark_code` | `text` | **CÓDIGO TÉCNICO SIAP** | Marca de Ocupación técnica SIAP. Acepta cualquier código numérico (`^[0-9]+$`) de una o más posiciones. Preserva el código crudo como texto sin rellenar ceros arbitrariamente (no convertir 0 en 00 ni 1 en 01). Valores observados en el archivo HGR No. 1 analizado: 0, 1, 5, 7, 9, 11, 20, 62, 63, 64, 65, 71, 73, 75, 77, 90, 98 y 99. Esta relación es evidencia del archivo, no un catálogo normativo completo. Todos los códigos numéricos se aceptan sin bloqueo ni advertencias. Valores no numéricos generan `MO_INVALID_FORMAT`. La interfaz muestra el código sin inventar su significado. |
...
| 27 | `Micro y grupo` | Micro Grupo, Microgrupo | `micro_group_code` | `text` | **CÓDIGO TÉCNICO SIAP** | Campo interno técnico del sistema SIAP. Valor observado en el archivo HGR No. 1 analizado: `5`. Se preserva como texto crudo sin inferencia laboral automática ni descomposición. Si viene vacío, se conserva vacío (`''`). Nunca se impone `5` como default en normalización ni persistencia. |
