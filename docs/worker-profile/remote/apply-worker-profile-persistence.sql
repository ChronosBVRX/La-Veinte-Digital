...
-- Aplicación remota controlada del perfil laboral. AUTÓNOMO.
-- Derivado exactamente de: supabase/migrations/20260804162446_worker_profile_persistence.sql
-- Adaptaciones: IF NOT EXISTS en tablas/columnas, rls_auto_enable respetado,
-- hardening de profiles ya aplicado (no se modifica).
...
-- ============================================================
DO $$ BEGIN IF current_database() != 'postgres' THEN RAISE EXCEPTION 'precondition: wrong database (%)', current_database(); END IF; END $$;
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='profiles') THEN RAISE EXCEPTION 'precondition: profiles missing'; END IF; END $$;
DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='payroll_contexts') THEN RAISE EXCEPTION 'precondition: payroll_contexts missing'; END IF; END $$;
DO $$ BEGIN IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='worker_preferences') THEN RAISE EXCEPTION 'precondition: worker_preferences already exists'; END IF; END $$;
DO $$ BEGIN IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='payroll_contexts' AND column_name='matricula') THEN RAISE EXCEPTION 'precondition: payroll_contexts.matricula already exists'; END IF; END $$;
--
-- Contiene:
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname='choose_basic_mode') THEN
    RAISE EXCEPTION 'verify failed: choose_basic_mode';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname='backfill_worker_profile') THEN
    RAISE EXCEPTION 'verify failed: backfill_worker_profile';
  END IF;
