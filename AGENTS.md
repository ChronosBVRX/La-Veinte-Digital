...
- Local (pre-auth) sync uses `@/shared/services/local-storage`; calculators hydrate from worker-context cache, with `/api/worker-context` (Supabase) as source of truth. Tarjetón import lives ONLY at `/profile/mi-informacion-laboral` (sección "Subir tarjetón IMSS"); `/tarjeton` y `/nomina/perfil` son redirects.
