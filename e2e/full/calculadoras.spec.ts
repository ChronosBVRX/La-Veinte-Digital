...
test.describe("Calculadoras - Indice", () => {
  test("carga la pagina de calculadoras", async ({ page }) => {
    ...
    // Check all calculator cards are visible
    await expect(
      page.getByRole("link", { name: "Aguinaldo" })
    ).toBeVisible()
    await expect(
      page.getByRole("link", { name: "Tiempo Extra" })
    ).toBeVisible()
    await expect(
      page.getByRole("link", { name: "Segunda de Julio" })
    ).toBeVisible()
    await expect(
      page.getByRole("link", { name: "Clausula 97" })
    ).toBeVisible()
    await expect(
      page.getByRole("link", { name: "Prestamos por Categoria" })
    ).toBeVisible()
  })
})
...
test.describe("Calculadoras - Aguinaldo", () => {
  test("carga la calculadora de aguinaldo", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Aguinaldo" })
    ).toBeVisible({ timeout: 10_000 })
  ...
  test("calcula aguinaldo con valores validos", async ({ page }) => {
    ...
    // Should see results
    await expect(
      page.getByText("Aguinaldo total estimado")
  ...
  test("calcula con valor cero correctamente", async ({ page }) => {
    ...
    // Should show results with zero for some values
    await expect(
      page.getByText("Aguinaldo total estimado")
    ).toBeVisible({ timeout: 5000 })
  })
...
test.describe("Calculadoras - Tiempo Extra", () => {
  test("carga la calculadora de tiempo extra", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Tiempo Extra" })
    ).toBeVisible({ timeout: 10_000 })
  ...
  test("recarga no altera otras calculadoras", async ({ page }) => {
    ...
    // Navigate to Aguinaldo
    await page.goto("/calculadoras/aguinaldo")
    await page.waitForLoadState("networkidle")

    // Aguinaldo should be at default state (not pre-filled)
...
test.describe("Calculadoras - Segunda de Julio", () => {
  test("carga la calculadora de segunda de julio", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Segunda de Julio" })
    ).toBeVisible({ timeout: 10_000 })
...
test.describe("Calculadoras - Clausula 97", () => {
  test("carga la calculadora de clausula 97", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Clausula 97" })
    ).toBeVisible({ timeout: 10_000 })
  })
})
