...
/**
 * Ayuda para Libros a Médicos (062).
 *
...
export const concept062Rule: PayrollRule = {
  ...
  calculate(ctx: PayrollRuleContext): RuleCalculationResult {
    ...
    const anchor = ctx.conceptAnchors.get("062")
    ...
    if (!eligible) {
      warnings.push("Requiere categoría médica, estomatólogo o cirujano maxilofacial autorizado")
    }
    if (anchor && eligible) {
    ...
    const concept: CalculatedPayrollConcept = {
      ...
      calculationSteps: [
        ...
        { label: "062 = base × 20%", expression: `${base} × 0.20 = ${formulaAmount}`, value: formulaAmount },
