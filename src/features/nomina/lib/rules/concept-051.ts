...
/**
 * Disponibilidad en Trasplantes (051).
 *
 ...
 */
export const concept051Rule: PayrollRule = {
  id: "051",
  version: "2.0.0",
  ...
  calculate(ctx: PayrollRuleContext): RuleCalculationResult {
    ...
    const anchor = ctx.conceptAnchors.get("051")
    const isRecurring = profile.recurringConcepts.some(
      (rc) => rc.conceptCode === "051" && rc.confirmed && rc.appearsNormally === true
    )
    const hasFact = profile.facts.some(
      (f) => f.key === "participates_in_transplant_program" && f.value === true
    )
    const hasPayslipEvidence = profile.facts.some(
      (f) => f.key === "concept_051_on_payslip" && f.value === true
    )
    ...
    if (!eligible) {
      warnings.push("Requiere incorporación formal a programa de trasplantes")
    }
    if (anchor && eligible) {
    ...
    const concept: CalculatedPayrollConcept = {
      code: "051",
      name: "Disponibilidad en Trasplantes",
      type: "earning",
      nature: "derived",
      ...
      calculationSteps: [
        ...
        { label: "051 = base × 20%", expression: `${base} × 0.20 = ${formulaAmount}`, value: formulaAmount },
        ...(anchor ? [{ label: "Último tarjetón (referencia)", expression: `Ancla: ${anchor.amount}`, value: anchor.amount }] : []),
      ],
      legalBasis: [{ source: "CCT", title: "Disponibilidad en Trasplantes", reference: "Cláusula aplicable del CCT" }],
      warnings,
    }
