...
export class ChatterboxEngine {
  ...
  autoRestarts = 0;
  /** watchdog: reinicio proactivo cada N bloques (degeneración de sesión larga observada en GTX 1650) */
  proactiveRestartEvery = 250;
  ...
  private processQueue() {
    ...
    this.pending[job.id] = async (r) => {
      ...
      const needsRestart =
        ...
        this.blocksSinceRestart >= this.proactiveRestartEvery;
  ...
  generate(text: string, voice: string): Promise<EngineGenerateResult> {
    const device = this.engineDevice();
    const key = blockCacheKey({ provider: "chatterbox-local", model: CHATTERBOX_MODEL_ID, device, voice, text });
    const id = `blk-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    return new Promise((resolve) => {
      this.queue.push({ id, text, voice, cacheKey: key, resolve });
      this.processQueue();
    });
  }
