...
import { rollbackLockerImportBatch } from "@/features/representacion/services/worker-importer/locker-importer";
...
export async function POST(req: Request): Promise<NextResponse> {
  ...
  try {
    const body = (await req.json()) as { batch_id?: string; delegation_id?: string };
    ...
    const result = await rollbackLockerImportBatch({
      batchId,
      delegationId,
      userId: auth.user.id,
    });
    ...
    const message = err instanceof Error ? err.message : "Error al revertir la importación de casilleros.";
    const status =
