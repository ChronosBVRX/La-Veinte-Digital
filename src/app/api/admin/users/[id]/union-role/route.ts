...
import { setUnionMembership } from "@/features/admin-users/services/admin-users-service"
import {
  adminJson,
  handleAdminError,
  parseUuidParam,
...
/**
 * Alta/baja del rol sindical (Representación) de una cuenta.
 * Ortogonal al rol de plataforma: solo gestiona `union_members` con auditoría.
 */
export async function POST(
  ...
  try {
    ...
    const parsed = unionRoleSchema.safeParse(await readJsonBody(request))
    if (!parsed.success) {
    ...
    return handleAdminError(error, {
      ...
      targetId: id ?? "",
      attemptedAction: "user.union_role_change",
      reason,
    })
  }
}
