...
import { resendConfirmationEmail } from "@/features/admin-users/services/admin-users-service"
...
export async function POST(
  ...
  try {
    ...
    const result = await resendConfirmationEmail(admin.user.id, targetId)
    return adminJson(result)
  } catch (error) {
    return handleAdminError(error, {
      actorId: admin.user.id,
      targetId: id ?? "",
      attemptedAction: "user.resend_confirmation",
    })
  }
}
