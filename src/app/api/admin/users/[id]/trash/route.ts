...
import { trashUser } from "@/features/admin-users/services/admin-users-service"
import {
  adminJson,
  handleAdminError,
  parseUuidParam,
...
export async function POST(
  ...
  try {
    ...
    const result = await trashUser(admin.user.id, targetId, parsed.data.reason)
    return adminJson(result)
  } catch (error) {
    return handleAdminError(error, {
      actorId: admin.user.id,
      targetId: id ?? "",
      attemptedAction: "user.trash",
      reason,
    })
  }
}
