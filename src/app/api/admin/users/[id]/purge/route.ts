...
import { purgeUser } from "@/features/admin-users/services/admin-users-service"
import {
  adminJson,
  handleAdminError,
  parseUuidParam,
...
export async function POST(
  ...
  try {
    ...
    const parsed = purgeSchema.safeParse(await readJsonBody(request))
    if (!parsed.success) {
      return adminJson({ error: "Datos inválidos", code: "invalid_request" }, 400)
    }

    reason = parsed.data.reason
    const result = await purgeUser(admin.user.id, targetId, parsed.data.confirmEmail, parsed.data.reason)
    return adminJson(result)
  } catch (error) {
    return handleAdminError(error, {
      actorId: admin.user.id,
      targetId: id ?? "",
      attemptedAction: "user.purge",
      reason,
    })
  }
}
